...
class OpenAICompleteMcpServer {
  ...
  private setupErrorHandling(): void {
    ...
    this.server.onerror = (error): void => {
      logger.error(
        "Server",
        `MCP Error: ${error instanceof Error ? error.stack : String(error)}`
      );
      throw error;
    };
  ...
  private async complete(
    ...
    try {
      ...
      logger.debug(
        "Completion",
        `Starting completion request for task ${taskId}`
      );
      ...
      if (
        error instanceof CompletionTimeoutError ||
        error instanceof CompletionCancelledError
      ) {
        throw error;
      }
  ...
  public cancelTask(taskId: string): boolean {
    ...
    if (
      task &&
      task.status !== CompletionStatusEnum.Complete &&
      task.status !== CompletionStatusEnum.Error
    ) {
      // Abort the request
...
function startServer() {
  ...
  if (!apiBaseUrl) {
    logger.error("Config", "OPENAI_API_BASE environment variable is required");
    process.exit(1);
  }
  if (!model) {
    logger.error("Config", "OPENAI_MODEL environment variable is required");
    process.exit(1);
  }
